...
//        Cart cart = userDetails.getUser().getCart();
    ...
    /*비회원 로그인 장바구니가 로그인한 유저의 장바구니로 전송*/
    public void saveCartToLoginUser(List<CartFromNonToLoginUserRequestDto> cartListDto, UserDetailsImpl userDetails) {

        for(CartFromNonToLoginUserRequestDto cart:cartListDto){
            System.out.println("들어온 총수량"+cart.getQuantity());
            Long productId=cart.getId(); //productId

            int quantity=cart.getQuantity();
            int sum=cart.getSum();
            CartRequestDto cartRequestDto=new CartRequestDto(quantity, sum);
            saveCart(productId, cartRequestDto, userDetails);
        }
//        for(CartFrom)
//        List<CartFromNonToLoginUserListRequestDto> cartList=cartListDto;
    }
}
